...
async def verify_same_story_with_typesafe(
    title_a: str,
    title_b: str,
    snippet_a: str = "",
    snippet_b: str = "",
    ...
    similarity between two news headlines/articles (0.35 <= sim <= 0.65).
    """
    ...
    payload = {
        ...
        "state": {
            "article_a": {
                "title": title_a,
                "summary": snippet_a[:300] if snippet_a else "",
            },
            "article_b": {
                "title": title_b,
                "summary": snippet_b[:300] if snippet_b else "",
            },
        },
        "questions": {
            "is_same_story": {
                "type": "noul",
                "instructions": "Do `article_a` and `article_b` describe the exact same underlying software engineering release, incident, or technical news event?",
            }
        },
    }
    try:
...
async def assign_article_cluster_async(
    ...
    new_title = new_article.vietnamese_title or new_article.title
    new_snippet = (
        getattr(new_article, "vietnamese_summary", None)
        or getattr(new_article, "content", "")
        or ""
    )
    new_topic_key = getattr(new_article, "cluster_topic_key", None)
    new_score = getattr(new_article, "relevance_score", 0.0) or 0.0
    ...
    for candidate in recent_candidates:
        ...
        if sim >= similarity_threshold and sim > best_similarity:
            ...
            cand_snippet = (
                getattr(candidate, "vietnamese_summary", None)
                or getattr(candidate, "content", "")
                or ""
            )
            borderline_candidates.append((sim, candidate, cand_title, cand_snippet))
    ...
    if not best_match and borderline_candidates and settings.TYPESAFE_API_KEY:
        ...
        _, top_cand, cand_title, cand_snippet = borderline_candidates[0]
        confirmed = await verify_same_story_with_typesafe(
            new_title, cand_title, snippet_a=new_snippet, snippet_b=cand_snippet
        )
        if confirmed:
            best_match = top_cand
