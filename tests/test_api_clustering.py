...
async def test_get_articles_group_duplicates():
    ...
    async with AsyncSessionLocal() as db:
        # Create test source
        src = Source(name=f"Test Source {rand_id}", url=f"https://source-{rand_id}.com/rss")
        db.add(src)
        await db.commit()
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        articles = res.json()
        cluster_articles = [a for a in articles if a.get("cluster_id") == f"cluster-{rand_id}"]
        ...
        assert cluster_articles[0]["related_articles"][0]["title"] == f"Tin vắn: Đã ra mắt sản phẩm A {rand_id}"

        # Test 2: group_duplicates=false
        res_all = await ac.get("/api/articles?group_duplicates=false")
        assert res_all.status_code == 200
        all_articles = res_all.json()
        cluster_all = [a for a in all_articles if a.get("cluster_id") == f"cluster-{rand_id}"]
